...
public class HeritageService {
    ...
    public Map<String, Object> registerUserWithEmail(String username, String email, String password) {
        Map<String, Object> result = new LinkedHashMap<>();
        if (username == null || username.trim().isEmpty() ||
            email == null || email.trim().isEmpty() ||
            password == null || password.trim().isEmpty()) {
            result.put("success", false);
            result.put("message", "Semua kolom pendaftaran harus diisi.");
            return result;
        }
        ...
        if (explorerRepository.findByUsername(cleanUsername).isPresent()) {
            result.put("success", false);
            result.put("message", "Username '" + cleanUsername + "' sudah terdaftar.");
            return result;
        }
        
        if (explorerRepository.findByEmail(cleanEmail).isPresent()) {
            result.put("success", false);
            result.put("message", "Email '" + cleanEmail + "' sudah terdaftar.");
            return result;
        }
    ...
    public Explorer loginOrRegisterGoogleUser(String email, String name) {
        if (email == null || email.trim().isEmpty()) {
            return null;
        }
        
        String cleanEmail = email.trim().toLowerCase();
        Optional<Explorer> existing = explorerRepository.findByEmail(cleanEmail);
        ...
        int count = 1;
        while (explorerRepository.findByUsername(finalUsername).isPresent()) {
        ...
        if (name != null && !name.trim().isEmpty()) {
            newExplorer.setName(name.trim());
        }
        return explorerRepository.save(newExplorer);
    }
