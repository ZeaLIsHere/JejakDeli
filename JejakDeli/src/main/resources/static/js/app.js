...
var otpCooldownTime = 0;
var otpInterval = null;

function resetPasswordFields() {
  var passFields = ["loginPassword", "registerPassword", "registerConfirmPassword"];
  ...
  // Clear usernames, emails, otp
  var textFields = ["loginUsername", "registerUsername", "registerEmail", "registerOtp"];
...
function sendOtp() {
  ...
  if (!email || !email.trim()) {
    showToast("Validasi Gagal", "Email Gmail harus diisi.", "error");
    return;
  }
  ...
  var url = "/api/auth/send-otp?email=" + encodeURIComponent(email);
  fetch(url, { method: "POST" })
    ...
    .then(function(res) {
      if (res.status === 200) {
        showToast("OTP Terkirim", res.body.message, "success");
        // Start countdown
        otpCooldownTime = 60;
        btn.textContent = "Kirim Ulang (" + otpCooldownTime + "s)";
        ...
        showToast("Gagal Kirim OTP", res.body.message || "Gagal mengirim kode OTP.", "error");
        btn.disabled = false;
        btn.textContent = "Kirim OTP";
      }
    })
    .catch(function() {
      showToast("Eror", "Terjadi kesalahan koneksi saat mengirim OTP.", "error");
      btn.disabled = false;
      btn.textContent = "Kirim OTP";
    });
}
...
function handleCredentialResponse(response) {
  ...
  var url = "/api/auth/google-login?token=" + encodeURIComponent(token);
  fetch(url, { method: "POST" })
    .then(function (r) {
      return r.json().then(data => ({ status: r.status, body: data }));
    })
    .then(function (res) {
      if (res.status === 200) {
        showToast("Login Google Berhasil", res.body.message, "success");
        showApp(res.body.username, res.body.isAdmin);
      } else {
        showToast("Login Google Gagal", res.body.message || "Gagal masuk menggunakan akun Google.", "error");
      }
    })
    .catch(function () {
      showToast("Eror", "Terjadi kesalahan sistem saat masuk dengan Google.", "error");
    });
}
...
function submitRegister(event) {
  ...
  var email = document.getElementById("registerEmail").value;
  var otp = document.getElementById("registerOtp").value;
  var p = document.getElementById("registerPassword").value;
  var cp = document.getElementById("registerConfirmPassword").value;
  ...
  if (!otp || !otp.trim()) {
    showToast(
      "Registrasi Gagal",
      "Kode OTP harus dimasukkan dan diverifikasi.",
      "error"
    );
    return;
  }
  ...
  var url =
    "/api/auth/register-with-otp?username=" +
    encodeURIComponent(u) +
    "&email=" +
